...
module.exports = {
  ...
  connect() {
    let options = { port: this.plugin.params.port };
    ...
    switch (this.plugin.params.transport) {
      ...
      case 'rtu':
        options = {
          baudRate: +this.plugin.params.baudRate,
          parity: this.plugin.params.parity,
          dataBits: this.plugin.params.dataBits,
          stopBits: this.plugin.params.stopBits
        };
        this.plugin.log('Connecting options ' + util.inspect(options), 1);
        return this.client.connectRTUBuffered(this.plugin.params.serialport, options);

      default:
        return Promise.reject({
          errno: 'Протокол ' + this.plugin.params.transport + ' в текущей версии не поддерживается'
        });
    }
  },
  ...
  getHostPortStr() {
    return this.plugin.params.transport== 'rtu' ? this.plugin.params.serialport : this.plugin.params.host + ':' + this.plugin.params.port;
  },
